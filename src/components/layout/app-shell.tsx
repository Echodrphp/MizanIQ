  ...
  return (
    <div className={cn("min-h-screen")} dir={isRTL ? "rtl" : "ltr"}>
      <div className="hidden lg:block">
        <Sidebar locale={locale} collapsed={sidebarCollapsed} onCollapseChange={setSidebarCollapsed} />
      </div>
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <div className="fixed inset-0 bg-black/50" onClick={() => setMobileMenuOpen(false)} />
          <div className={cn("fixed top-0 h-full", isRTL ? "right-0" : "left-0")}>
            <Sidebar locale={locale} collapsed={false} onCollapseChange={() => {}} />
          </div>
        </div>
      )}
      <div className={cn(
