...
const performanceMetrics: MetricData[] = [
  { label: "Total Spend", value: 28450, formattedValue: "$28,450", trend: { direction: "up", percentage: 12.5, isPositive: false }, meta: { ...DEMO_META, confidence: "high" } },
  { label: "Revenue", value: 96800, formattedValue: "$96,800", trend: { direction: "up", percentage: 8.3, isPositive: true }, meta: { ...DEMO_META, confidence: "high" } },
  { label: "ROAS", value: 3.4, formattedValue: "3.40x", trend: { direction: "down", percentage: 4.2, isPositive: false }, target: 4.0, meta: { ...DEMO_META, confidence: "medium" } },
  { label: "Purchases", value: 142, formattedValue: "142", trend: { direction: "up", percentage: 6.1, isPositive: true }, meta: { ...DEMO_META, confidence: "high" } },
  { label: "Leads", value: 387, formattedValue: "387", trend: { direction: "up", percentage: 15.2, isPositive: true }, meta: { ...DEMO_META, confidence: "medium" } },
  { label: "CPA", value: 200, formattedValue: "$200", trend: { direction: "up", percentage: 5.8, isPositive: false }, target: 180, meta: { ...DEMO_META, confidence: "high" } },
  { label: "CTR", value: 2.8, formattedValue: "2.8%", trend: { direction: "down", percentage: 3.1, isPositive: false }, meta: { ...DEMO_META, confidence: "high" } },
  { label: "CPM", value: 12.5, formattedValue: "$12.50", trend: { direction: "up", percentage: 8.0, isPositive: false }, meta: { ...DEMO_META, confidence: "high" } },
];
...
const weeklyData = [
  { week: "Week 1", spend: 6200, revenue: 22000, roas: 3.55 },
  { week: "Week 2", spend: 7100, revenue: 25500, roas: 3.59 },
  { week: "Week 3", spend: 7800, revenue: 24800, roas: 3.18 },
  { week: "Week 4", spend: 7350, revenue: 24500, roas: 3.33 },
];
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t("title")} description={t("description")}>
        <Badge variant="demo" className="gap-1.5"><span className="h-2 w-2 rounded-full bg-amber-500 animate-pulse" />Demo Data</Badge>
      </PageHeader>

      {/* Key Metrics */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {performanceMetrics.slice(0, 4).map((m) => <MetricCard key={m.label} metric={m} />)}
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {performanceMetrics.slice(4).map((m) => <MetricCard key={m.label} metric={m} />)}
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-neutral-100 dark:border-[#2A3544] bg-neutral-50/50 dark:bg-[#0F1419]/50">
                  ...
                  <th className="text-start py-3 px-5 text-[10px] font-bold uppercase tracking-wider text-neutral-500">Spend</th>
                  <th className="text-start py-3 px-5 text-[10px] font-bold uppercase tracking-wider text-neutral-500">Revenue</th>
                  <th className="text-start py-3 px-5 text-[10px] font-bold uppercase tracking-wider text-neutral-500">ROAS</th>
                  <th className="text-start py-3 px-5 text-[10px] font-bold uppercase tracking-wider text-neutral-500">Share</th>
                  <th className="text-start py-3 px-5 text-[10px] font-bold uppercase tracking-wider text-neutral-500">Trend</th>
                </tr>
              </thead>
              <tbody>
                {platformBreakdown.map((p) => (
                  <tr key={p.platform} className="border-b border-neutral-100 dark:border-[#2A3544] hover:bg-sky-50/30 dark:hover:bg-[#0F1419] transition-colors">
                    ...
                    <td className="py-3.5 px-5">
                      <span className="font-bold text-neutral-900 dark:text-white">{p.roas.toFixed(2)}x</span>
                    </td>
                    <td className="py-3.5 px-5">
            ...
            {weeklyData.map((w, i) => (
              <div key={w.week} className="rounded-xl bg-gradient-to-br from-sky-50 to-cyan-50 dark:from-sky-500/5 dark:to-cyan-500/5 border border-sky-100 dark:border-[#2A3544] p-4 hover:shadow-md hover:shadow-sky-100 dark:hover:shadow-none transition-shadow">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-xs font-semibold text-sky-700 dark:text-sky-400">{w.week}</p>
